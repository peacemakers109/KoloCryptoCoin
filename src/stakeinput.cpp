...
// The zNWC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zNWC is a hash of the serial
...
{
    //Create an output returning the zNWC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZNWCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zNWC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zNWC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZNWCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zNWC output", __func__);
...
{
    CzNWCTracker* znwcTracker = pwallet->znwcTracker.get();
    CMintMeta meta;
    if (!znwcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    znwcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a NWC stake is the outpoint
