...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zNWC stake block is less than allowed start height", __func__);

    if (CZPivStake* zNWC = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zNWC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zNWC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zNWC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zNWC->GetValue()));
        if (nChecksum200 != zNWC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zNWC->GetChecksum(), nChecksum200);
    } else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zNWC fails context checks", __func__);
    } else {
        ...
        CPivStake* nwcInput = new CPivStake();
        nwcInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(nwcInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zNWC is checked above in ContextualCheckZerocoinStake()
