...
#include "ui_znwccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->znwcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->znwcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zNWC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNWC seed used to mint this zNWC is not the same as currently hold in the wallet";
            else
