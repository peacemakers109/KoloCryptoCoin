...
CzNWCWallet::CzNWCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znwc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNWCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNWCSeed_deprecated()) {
                    LogPrintf("%s: Updated zNWC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znwc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znwc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNWCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNWCCount(nCountLastUsed);
    else if (!walletdb.ReadZNWCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNWCWallet::Lock()
{
    seedMaster = 0;
}

void CzNWCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNWCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNWC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNWCWallet::LoadMintPoolFromDB()
{
...
void CzNWCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNWCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNWCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znwcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNWCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNWC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znwcTracker which also adds to database
    pwalletMain->znwcTracker->Add(dMint, true);
...
void CzNWCWallet::SeedToZNWC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNWCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNWCWallet::GenerateDeterministicZNWC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNWCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNWCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNWCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
