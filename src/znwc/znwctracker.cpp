...
CzNWCTracker::CzNWCTracker(std::string strWalletFile)
{
...
CzNWCTracker::~CzNWCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNWCTracker::Init()
{
...
bool CzNWCTracker::Archive(CMintMeta& meta)
{
...
bool CzNWCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNWCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNWCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNWCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzNWCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzNWCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzNWCTracker::GetSerialHashes()
{
...
CAmount CzNWCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNWCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNWCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNWCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNWCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNWCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNWCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNWCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNWCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNWCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNWCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNWCWallet* zNWCWallet)
{
    bool iszNWCWalletInitialized = (NULL != zNWCWallet);
    CMintMeta meta;
    ...
    if (! iszNWCWalletInitialized)
        zNWCWallet = new CzNWCWallet(strWalletFile);
    meta.isSeedCorrect = zNWCWallet->CheckSeed(dMint);
    if (! iszNWCWalletInitialized)
        delete zNWCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNWCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNWCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNWCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNWCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNWCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNWCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNWCWallet* zNWCWallet = new CzNWCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNWCWallet);
        }
        delete zNWCWallet;
        LogPrint("zero", "%s: added %d dznwc from DB\n", __func__, listDeterministicDB.size());
    }
