...
/**
 * NWCCurrency RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZnwcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setznwcseed(const UniValue& params, bool fHelp);
extern UniValue getznwcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdznwc(const UniValue& params, bool fHelp);
extern UniValue dznwcstate(const UniValue& params, bool fHelp);
